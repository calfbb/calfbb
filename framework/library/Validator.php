...
//    }
    ...
    public function validate($key, $recursive = false, $label = '') {
        ...
        $this->fields[$key] = (empty($label)) ? '字段名 "' . $key . '"' : $label;
    ...
    protected function _getDefaultMessage($rule, $args = null) {
        ...
        switch ($rule) {
            case 'email':
                $message = '%s 字段值不是一个正确的邮箱地址.';
                break;

            case 'ip':
                $message = '%s 字段值不是一个正确的IP地址.';
                break;

            case 'url':
                $message = '%s  字段值不是一个正确的url地址 .';
                break;

            case 'required':
                $message = '%s 字段值是必须的，不能为空.';
                break;

            case 'float':
                $message = '%s 字段值不是一个正确的浮点型float.';
                break;

            case 'integer':
                $message = '%s 必须是一个整型（integer）的值';
                break;
